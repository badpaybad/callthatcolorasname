...
public class SoundStreamPlugin : FlutterPlugin,
        ...
        ActivityAware {
    public val logTag = "SoundStreamPlugin"
    public val audioRecordPermissionCode = 14887
    ...
    public var permissionToRecordAudio: Boolean = false
    public var activeResult: Result? = null
    public var debugLogging: Boolean = false

    //========= Recorder's vars
    public val mRecordFormat = AudioFormat.ENCODING_PCM_16BIT
    public var mRecordSampleRate = 16000 // 16Khz
    public var mRecorderBufferSize = 8192
    public var mPeriodFrames = 8192
    public var audioData: ShortArray? = null
    public var mRecorder: AudioRecord? = null
    public var mListener: OnRecordPositionUpdateListener? = null

    //========= Player's vars
    public var mAudioTrack: AudioTrack? = null
    public var mPlayerSampleRate = 16000 // 16Khz
    public var mPlayerBufferSize = 10240
    public var mPlayerFormat: AudioFormat = AudioFormat.Builder()
    ...
    public fun onAttachedToEngine(applicationContext: Context, messenger: BinaryMessenger) {
...
//        currentActivity = null
    ...
    public fun hasRecordPermission(): Boolean {
    ...
    public fun hasPermission(result: Result) {
        result.success(hasRecordPermission())
    }

    public fun requestRecordPermission() {
    ...
    public fun initializeRecorder(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    public fun completeInitializeRecorder() {
    ...
    public fun sendEventMethod(name: String, data: Any) {
    ...
    public fun debugLog(msg: String) {
        if (debugLogging) {
            Log.d(logTag, msg)
        }
    }

    public fun startRecording(result: Result) {
        try {
    ...
    public fun stopRecording(result: Result) {
        try {
    ...
    public fun sendRecorderStatus(status: SoundStreamStatus) {
        sendEventMethod("recorderStatus", status.name)
    }

    public fun initializePlayer(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    public fun writeChunk(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    public fun pushPlayerChunk(chunk: ByteArray, result: Result) {
        try {
    ...
    public fun startPlayer(result: Result) {
        try {
    ...
    public fun stopPlayer(result: Result) {
        try {
            if (mAudioTrack?.state == AudioTrack.STATE_INITIALIZED) {
                mAudioTrack?.stop()
            }
    ...
    public fun sendPlayerStatus(status: SoundStreamStatus) {
        sendEventMethod("playerStatus", status.name)
    }

    public fun createRecordListener(): OnRecordPositionUpdateListener? {
