...
class ColorHelper {
  ...
  static ColorHelper instance = ColorHelper._();

  ColorHelper._() {
    for (var i = 0; i < this.names.length; i++) {
      var color = "#${this.names[i][0]}";
      //print("color $color");
      var rgb = this.hexToRgb(color);
      var hsl = this.hexToHslRgb(color);
      this.names[i].addAll([
        rgb[0].toString(),
        rgb[1].toString(),
        rgb[2].toString(),
  ...
  List<dynamic> convertToMatrixRgb(Uint8List imgInJpg,
      {int valueBrigthness = -1}) {
    final decodedImage = DartImage.decodeJpg(imgInJpg);
    List<dynamic> imgArr = [];
    for (var y = 0; y < decodedImage!.height; ++y) {
      for (var x = 0; x < decodedImage.width; ++x) {
        ...
        if (valueBrigthness > 0) {
          red = (red + valueBrigthness).clamp(0, 255);
          green = (green + valueBrigthness).clamp(0, 255);
          blue = (blue + valueBrigthness).clamp(0, 255);
        }

        imgArr.add([red, green, blue]);
        decodedImage.setPixel(x, y, ColorRgb8(red, green, blue));
      }
    }

    // final decodedBytes = decodedImage!.getBytes ();
    // List<dynamic> imgArr = [];
  ...
  name(String colorHexString) {
    ...
    if (colorHexString.length < 3 || colorHexString.length > 7) {
      return ["#000000", "Invalid Color: " + colorHexString, "#000000", "", false];
    }
    if (colorHexString.length % 3 == 0) {
      colorHexString = "#" + colorHexString;
    }
    if (colorHexString.length == 4) {
      colorHexString = "#" +
          colorHexString.substring(1, 1) +
          colorHexString.substring(1, 1) +
          colorHexString.substring(2, 1) +
          colorHexString.substring(2, 1) +
          colorHexString.substring(3, 1) +
          colorHexString.substring(3, 1);
    }

    var rgb = hexToRgb(colorHexString);
    var r = rgb[0], g = rgb[1], b = rgb[2];
    var hsl = hexToHslRgb(colorHexString);
    ...
    for (var i = 0; i < this.names.length; i++) {
      if (colorHexString == "#" + this.names[i][0])
        return [
          "#" + this.names[i][0],
          this.names[i][1],
      ...
      ndf1 = Math.pow(r - int.parse(this.names[i][3]), 2).toDouble() +
          Math.pow(g - int.parse(this.names[i][4]), 2).toDouble() +
          Math.pow(b - int.parse(this.names[i][5]), 2).toDouble();
      ndf2 = (Math.pow(h - int.parse(this.names[i][6]), 2)).abs().toDouble() +
          Math.pow(s - int.parse(this.names[i][7]), 2) +
          (Math.pow(l - int.parse(this.names[i][8]), 2)).abs().toDouble();
      ndf = ndf1 + ndf2 * 2;
      if (df < 0 || df > ndf) {
        df = ndf;
        cl = i;
    ...
    return (cl < 0
        ? ["#000000", "Invalid Color: " + colorHexString, "#000000", "", false]
        : [
            "#" + this.names[cl][0],
            this.names[cl][1],
  ...
  String shadergb(String shadename) {
  ...
  List<int> hexToRgb(String hexcolor) {
    try {
      return [
        int.parse("0x${hexcolor.substring(1, 3)}"),
        int.parse("0x${hexcolor.substring(3, 5)}"),
        int.parse("0x${hexcolor.substring(5, 7)}")
      ];
    } catch (ex) {
      print("$hexcolor ERR $ex");
      return [];
    }
  }

  List<int> hexToHslRgb(String hexcolor) {
    try {
      var rgb = [
        int.parse('0x' + hexcolor.substring(1, 3)) / 255,
        int.parse('0x' + hexcolor.substring(3, 5)) / 255,
        int.parse('0x' + hexcolor.substring(5, 7)) / 255
      ];
      ...
      return [(h * 255).toInt(), (s * 255).toInt(), (l * 255).toInt()];
    } catch (ex) {
      print("$hexcolor ERR $ex");
      return [];
    }
...
//http://kleur.werkvanbart.nl/color-name-hue-tool/js/ntc.js
  List<dynamic> names = [
    ...
    ["483D8B", "Dark Slate Blue", "Purple"],
    ["663399", "Rebecca Purple", "Purple"],
    ["4B0082", "Indigo", "Purple"],
    ["FFA07A", "Light Salmon", "Red"],
    ["FA8072", "Salmon", "Red"],
    ["E9967A", "Dark Salmon", "Red"],
    ...
    ["DAA520", "Golden Rod", "Brown"],
    ["B8860B", "Dark Golden Rod", "Brown"],
    ["CD853F", "Peru", "Brown"],
